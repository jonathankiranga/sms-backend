...
async function applyParentPayment(db, phone, amount, activeSchoolId) {
  ...
  // Resolve school for expiry calculation
  ...
  // Walk forward term-by-term, chaining each expiry off the previous one so
  // multi-term payments produce the correct final expiry date.
  let cursor = new Date();
  let termsActivated = 0;
  let finalExpiry = cursor;

  while (remaining >= termPrice) {
    remaining -= termPrice;
    termsActivated += 1;
    // Advance one term past the current cursor
    if (schoolId) {
      // getNextTermStart finds the next term start *after* cursor — pass it the date
      finalExpiry = await getNextTermStart(db, schoolId, cursor);
    } else {
      finalExpiry = new Date(cursor.getTime() + 90 * 86400000);
    }
    cursor = finalExpiry;
  }

  if (termsActivated === 0) {
    // Amount was less than one term — activate for one term anyway and bank no credit
    finalExpiry = schoolId
      ? await getNextTermStart(db, schoolId, new Date())
  ...
  await db.execute(
    ...
    [activeSchoolId || null, phone, currentTerm, currentYear, amount, finalExpiry]
  );
}
